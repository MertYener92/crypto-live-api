...
app.get('/gold-prices', (req, res) => {
  ...
  res.json({
    ALTIN: {
      ...
      price: gramTry, priceUsd: goldData.xauusd, usdtry: goldData.usdtry,
      change: goldData.change, high24h: goldData.high24hTry || gramTry, low24h: goldData.low24hTry || gramTry,
      updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h => h.price || gramTry),
    },
    XAUUSD: {
      symbol: 'XAUUSD', name: 'Ons Altın',
      price: goldData.xauusd, change: goldData.change,
      high24h: goldData.xauusd, low24h: goldData.xauusd, updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h =>
        h.price && goldData.usdtry > 0 ? Number(((h.price * 31.1035) / goldData.usdtry).toFixed(2)) : goldData.xauusd
      ),
    },
    CEYREK_YENI: {
      symbol: 'CEYREK_YENI', name: 'Çeyrek Altın',
      price: getCeyrek(), change: goldData.change, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h => Number(((h.price * goldData.usdtry) / 31.1035 * 1.75).toFixed(2))),
    },
    YARIM_YENI: {
      symbol: 'YARIM_YENI', name: 'Yarım Altın',
      price: getYarim(), change: goldData.change, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h => Number(((h.price * goldData.usdtry) / 31.1035 * 3.5).toFixed(2))),
    },
    TEK_YENI: {
      symbol: 'TEK_YENI', name: 'Tam Altın',
      price: getTam(), change: goldData.change, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h => Number(((h.price * goldData.usdtry) / 31.1035 * 7).toFixed(2))),
    },
    CUM_ALTIN: {
      symbol: 'CUM_ALTIN', name: 'Cumhuriyet Altını',
      price: sarrafiyeData.CUM_ALTIN?.price || Number((gramTry * 7.2).toFixed(2)),
      change: goldData.change, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: goldIntraday.slice(-24).map(h => Number(((h.price * goldData.usdtry) / 31.1035 * 7.2).toFixed(2))),
    },
    ATA_ALTIN: {
    ...
    GUMUS: {
      symbol: 'GUMUS', name: 'Gram Gümüş',
      price: sarrafiyeData.GUMUS?.price || goldData.gramSilverTry,
      change: goldData.silverChange, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: silverIntraday.slice(-24).map(h => h.price),
    },
    PLATIN: {
      symbol: 'PLATIN', name: 'Gram Platin',
      price: sarrafiyeData.PLATIN?.price || 0,
      change: sarrafiyeData.PLATIN?.change || 0, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
      sparkline: platinIntraday.slice(-24).map(h => h.price),
    },
    PALADYUM: {
      symbol: 'PALADYUM', name: 'Gram Paladyum',
      price: sarrafiyeData.PALADYUM?.price || 0,
      change: sarrafiyeData.PALADYUM?.change || 0, high24h: 0, low24h: 0, updatedAt: goldData.updatedAt,
...
app.get('/chart/gold/:symbol', (req, res) => {
  ...
  if (symbol === 'GUMUS') {
    ...
    if (h.length === 0 && goldData.gramSilverTry > 0) {
      const now = Math.floor(Date.now() / 1000);
      return res.json([{ time: now - 3600, price: goldData.gramSilverTry }, { time: now, price: goldData.gramSilverTry }]);
    }
    const data = [...h];
    if (data.length > 0 && goldData.gramSilverTry > 0)
  ...
  if (history.length === 0) {
    if (goldData.gramTry > 0) {
      const now = Math.floor(Date.now() / 1000);
      return res.json([{ time: now - 3600, price: goldData.gramTry }, { time: now, price: goldData.gramTry }]);
    }
    ...
    if (response.data && response.data.length > 0) {
      response.data.forEach(row => {
        coinMetadata[row.symbol] = {
          symbol: row.symbol, name: row.name || row.symbol,
          logo: row.logo_url || '', geckoId: row.gecko_id || '',
          rank: 9999, marketCap: 0,
        };
      });
      ...
      try {
        ...
        response.data.forEach(coin => {
          ...
          if (symbolSet.has(symbol)) {
            collected.push({ symbol, name: coin.name, rank: coin.market_cap_rank || 9999, marketCap: Number(coin.market_cap || 0), logo: coin.image || '', geckoId: coin.id });
          }
        });
  ...
  ws.on('message', msg => {
    try {
      ...
      prices[symbol]  = {
        ...
        marketCap: meta.marketCap || 0, dominance,
...
app.get('/fund/comparison/:code', async (req, res) => {
  try {
    ...
    const benchmarks = items
      ...
      .map(i => ({ code: i.fonKodu, name: labelMap[i.fonKodu] || i.fonUnvan || i.fonKodu, return: Number((i.fonTurGetiri * 100).toFixed(2)) }))
...
app.get('/fund/returns/:code', async (req, res) => {
  try {
    ...
    const pricesList = items
      .map(d => ({ date: new Date(d.tarih || d.TARIH || d.date), price: parseFloat(String(d.fiyat || d.FIYAT || d.birimPayDegeri || 0).replace(',', '.')) }))
      .filter(d => d.price > 0)
      .sort((a, b) => a.date - b.date);
    const current = pricesList[pricesList.length - 1].price;
    const today   = new Date();
    function getReturn(months) {
      const target = new Date(today);
      target.setMonth(target.getMonth() - months);
      const found = pricesList.find(p => p.date >= target);
...
app.get('/fund/sync', async (req, res) => {
  ...
  try {
    const tefasData = await axios.post('https://www.tefas.gov.tr/api/funds/getFplFonList', {}, {
      timeout: 15000,
      headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'Origin': 'https://www.tefas.gov.tr', 'Referer': 'https://www.tefas.gov.tr/BESSorgu.aspx', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'sec-fetch-site': 'same-origin', 'sec-fetch-mode': 'cors' }
    });
    ...
    if (items.length > 0) {
      ...
      for (const item of items) {
        ...
        if (!code) continue;
        try {
          await axios.post(`${SUPABASE_URL}/rest/v1/assets`,
            { symbol: code, name, type: 'fund', provider: 'tefas', currency: 'TRY', search_keywords: `${code.toLowerCase()} ${name.toLowerCase()} bes fon emeklilik`, is_active: true },
...
app.get('/fund/search', async (req, res) => {
  try {
    ...
    const funds = items
      ...
      .map(d => ({ code: d.fonKodu || d.FONKODU, name: d.fonUnvan || d.FONUNVAN || d.fonAdi || d.fonKodu, type: d.fonTuru || d.FONTURU || '' }));
